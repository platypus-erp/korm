...
import org.chmuche.korm.sql.column.type.AutoIncColumnType
import org.chmuche.korm.sql.column.type.BinaryColumnType
import org.chmuche.korm.sql.column.type.BlobColumnType
import org.chmuche.korm.sql.column.type.BooleanColumnType
import org.chmuche.korm.sql.column.type.CharacterColumnType
import org.chmuche.korm.sql.column.type.ColumnType
import org.chmuche.korm.sql.column.type.DateColumnType
import org.chmuche.korm.sql.column.type.DateTimeColumnType
import org.chmuche.korm.sql.column.type.DecimalColumnType
import org.chmuche.korm.sql.column.type.EntityIDColumnType
import org.chmuche.korm.sql.column.type.EnumerationColumnType
import org.chmuche.korm.sql.column.type.EnumerationNameColumnType
import org.chmuche.korm.sql.column.type.IColumnType
import org.chmuche.korm.sql.column.type.IntegerColumnType
import org.chmuche.korm.sql.column.type.LongColumnType
import org.chmuche.korm.sql.column.type.StringColumnType
import org.chmuche.korm.sql.column.type.TimeColumnType
import org.chmuche.korm.sql.column.type.UUIDColumnType
import org.chmuche.korm.sql.transactions.TransactionManager
import org.chmuche.korm.sql.vendors.currentDialect
import org.chmuche.korm.sql.vendors.inProperCase
...
open class Table(name: String = "") : ColumnSet(), DdlAware {
    ...
    private fun <T> Column<T>.clone(columntype: IColumnType): Column<T> = Column(this@Table, this.name, columnType)
    ...
    fun datetime(name: String): Column<LocalDateTime> = registerColumn(name, DateTimeColumnType())

    fun time(name: String): Column<LocalTime> = registerColumn(name, TimeColumnType())

    /**
     * @sample org.chmuche.korm.sql.tests.shared.EntityTests.testBlobField
     */
    ...
    fun varchar(name: String, length: Int = 42, collate: String? = null): Column<String> = registerColumn(name, StringColumnType(length, collate))

    private fun <T> Column<T>.cloneWithAutoInc(idSeqName: String?): Column<T> = when (columnType) {
        is AutoIncColumnType -> this
        is ColumnType ->
            this@cloneWithAutoInc.clone(AutoIncColumnType(columnType, idSeqName ?: "${tableName}_${name}_seq"))
