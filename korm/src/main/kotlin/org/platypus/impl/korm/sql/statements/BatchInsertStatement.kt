...
open class BatchInsertStatement(table: KormTable, ignore: Boolean = false): InsertStatement<List<Map<Column<*>, Any>>>(table, ignore) {
    ...
    private fun Column<*>.isDefaultable() = columnType.required || defaultValueFun != null
    ...
    protected var arguments: List<List<Pair<Column<*>, Any?>>>? = null
        get() = field ?: run {
            val nullableColumns = allColumnsInDataSet().filter { it.columnType.required }
            data.map { single ->
