...
class Alias<out T : Table>(val delegate: T, val alias: String) : KormTable(alias) {
    ...
    operator fun <T : Any?> get(original: Column<T>): Column<T> = delegate.columns.find { it == original }?.let { it.clone() as? Column<T> } ?: error("TableColumn not found in original table")
}
...
class QueryAlias(val query: Query, val alias: String) : ColumnSet {
    ...
    operator fun <T : Any?> get(original: Column<T>): Column<T> = query.set.source.columns.find { it == original }?.
            let { it.clone() as? Column<T> } ?: error("TableColumn not found in original table")
...
fun <T : KormTable> T.alias(alias: String) = Alias(this, alias)
fun <T : Query> T.alias(alias: String) = QueryAlias(this, alias)
fun <T : Expression<*>> T.alias(alias: String) = ExpressionAlias(this, alias)

fun Join.joinQuery(on: ((QueryAlias) -> Op<Boolean>), joinType: JoinType = JoinType.INNER, joinPart: () -> Query): Join {
...
fun KormTable.joinQuery(on: ((QueryAlias) -> Op<Boolean>), joinType: JoinType = JoinType.INNER, joinPart: () -> Query)
