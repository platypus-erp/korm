 ...
 */
open class InsertStatement<Key : Any>(val table: KormTable, val isIgnore: Boolean = false) : UpdateBuilder<Int>(StatementType.INSERT, listOf(table)) {
    ...
    infix operator fun get(pk: KormPkProperty<*>): ModelID = generatedKey as? ModelID
            ?: error("No key generated")
    infix operator fun <T : Key> get(pk: KormProperty<*, T>): T = generatedKey as? T ?: error("No key generated")
    ...
    open protected fun valuesAndDefaults(values: Map<Column<*>, Any?> = this.values): Map<Column<*>, Any?> {
        val columnsWithNotNullDefault = targets.flatMap { it.columns }.filter {
            (it.dbDefaultValue != null || it.defaultValueFun != null) && !it.columnType.required && it !in values.keys
        }
