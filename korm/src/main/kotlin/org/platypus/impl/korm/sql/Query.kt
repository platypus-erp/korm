...
import org.platypus.impl.korm.model.IdTable
...
class ResultRow(size: Int, private val fieldIndex: Map<Expression<*>, Int>) {
    ...
    operator fun <T> get(c: Expression<T>): T {
        ...
        return d?.let {
            ...
            (c as? ExpressionWithColumnType<T>)?.columnType?.valueFromDB(it) ?: it ?: run {
                ...
                if (column?.dbDefaultValue != null && column.columnType.required == false) {
                    exposedLogger.warn("TableColumn ${TransactionManager.current().identity(column)} is marked as not null, " +
    ...
    operator fun <M : BaseModel<M, *>, T> get(prop: KormProperty<M, T>): T = get(prop.column)
    ...
    companion object {
        ...
        internal fun create(columns: List<Column<*>>): ResultRow =
                ResultRow(columns.size, columns.mapIndexed { i, c -> c to i }.toMap()).apply {
                    columns.forEach {
                        this[it] = it.defaultValueFun?.invoke() ?: if (!it.columnType.required) NotInitializedValue else null
                    }
                }
    }
}
...
open class ModelQuery<M : BaseModel<M, *>>(private val env: PlatypusEnvironement, m: M, where: Op<Boolean>?) : Query(env.cr, m.internalTable, where)
