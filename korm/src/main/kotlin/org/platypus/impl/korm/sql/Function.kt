...
class Count(val expr: Expression<*>, val distinct: Boolean = false) : Function<Int>() {
    ...
    override val columnType: IColumnType = IntegerColumnType(false)
}
...
class Substring(val expr: Expression<String?>, val start: ExpressionWithColumnType<Int>, val length: ExpressionWithColumnType<Int>) : Function<String>() {
    override fun toSQL(queryBuilder: QueryBuilder): String = currentDialect.functionProvider.substring(expr, start, length, queryBuilder)
...
class Random(val seed: Int? = null) : Function<BigDecimal>() {
    override fun toSQL(queryBuilder: QueryBuilder): String = currentDialect.functionProvider.random(seed)
...
class Cast<out T>(val expr: Expression<*>, override val columnType: IColumnType) : Function<T?>() {
    override fun toSQL(queryBuilder: QueryBuilder): String = currentDialect.functionProvider.cast(expr, columnType, queryBuilder)
}
...
class SqlConcat(vararg val expr: Expression<*>, val separator: String? = null, val prefix: Expression<String>? = null, val suffix: Expression<String>? = null) : Function<String>() {
    override fun toSQL(queryBuilder: QueryBuilder): String = buildString {
        val iter = expr.iterator()
        if (prefix != null) {
            append(prefix.toSQL(queryBuilder))
            append(" || ")
        }
        if (iter.hasNext()) {
            append(iter.next().toSQL(queryBuilder))
        }
        while (iter.hasNext()) {
            append(" || ")
            if (separator != null && separator.isNotBlank()) {
                append(stringLiteral(separator).toSQL(queryBuilder))
            }
            append(iter.next().toSQL(queryBuilder))
        }
        if (suffix != null) {
            append(" || ")
            append(suffix.toSQL(queryBuilder))
        }
    }
