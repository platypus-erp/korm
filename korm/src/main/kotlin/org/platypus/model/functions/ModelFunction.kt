...
class ModelFunction<M : Model<M>>(private val m: M) {
    ...
    fun compute(function: (Record<M>) -> PublicApiReturn<Unit>) = ApiOneNoParamStacker(function).apply {
        methodName = m.modelName + " compute"
    }

    fun onChange(vararg fieldOnChange: IModelField<M, *>, function: (Record<M>) -> PublicApiReturn<Unit>) = ApiOneNoParamStacker(function).apply {
        methodName = m.modelName + " compute"
    }
}
