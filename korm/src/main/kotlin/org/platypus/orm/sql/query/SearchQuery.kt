...
interface SearchQuery<M : IModel<M>> {
    ...
    fun where(predicate: SearchQueryWherePart<M>.(M) -> Expression<Boolean>): SearchQuery<M>
    fun or(predicate: SearchQueryWherePart<M>.(M) -> Expression<Boolean>): SearchQuery<M>
    fun and(predicate: SearchQueryWherePart<M>.(M) -> Expression<Boolean>): SearchQuery<M>
