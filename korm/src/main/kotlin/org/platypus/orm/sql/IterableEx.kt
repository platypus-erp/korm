...
interface SizedIterable<out T> : Iterable<T> {
    fun limit(limit: Int, offset: Int = 0): SizedIterable<T>
    fun count(): Int
    fun empty(): Boolean
...
class EmptySizedIterable<out T> : SizedIterable<T>, Iterator<T> {
    ...
    override fun limit(limit: Int, offset: Int): SizedIterable<T> = this
...
class SizedCollection<out T>(val delegate: Collection<T>) : SizedIterable<T> {
    override fun limit(limit: Int, offset: Int): SizedIterable<T> = SizedCollection(delegate.drop(offset).take(limit))
...
class LazySizedCollection<out T>(val delegate: SizedIterable<T>) : SizedIterable<T> {
    ...
    override fun limit(limit: Int, offset: Int): SizedIterable<T> = delegate.limit(limit, offset)
...
infix fun <T, R> SizedIterable<T>.mapLazy(f: (T) -> R): SizedIterable<R> {
    ...
    return object : SizedIterable<R> {
        override fun limit(limit: Int, offset: Int): SizedIterable<R> = source.limit(limit, offset).mapLazy(f)
